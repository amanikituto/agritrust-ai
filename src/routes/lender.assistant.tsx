...
function AssistantPage() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: "assistant", content: "I'm your lending copilot. Ask me to summarize the queue, compare applicants, or explain a recommendation." },
  ]);
  ...
  const send = useMutation({
    mutationFn: async (text: string) => {
      ...
      setMessages(next);
      const res = await ask({ data: { messages: next, role: "lender" } });
  ...
  return (
    <div className="space-y-6">
      <SectionTitle eyebrow="Copilot" title="AI Assistant" sub="Grounded on live portfolio data" />
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
              <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full ${m.role === "assistant" ? "bg-gradient-to-br from-emerald to-violet" : "bg-surface-elevated"}`}>
                {m.role === "assistant" ? <Sparkles className="h-4 w-4 text-primary-foreground" /> : <span className="text-xs font-bold">L</span>}
              </span>
        ...
        <div className="border-t border-border/60 p-4">
          <div className="flex items-center gap-2 rounded-xl bg-surface-elevated/60 px-3 py-2">
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()}
              className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" placeholder="Ask the copilot…" />
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {["Summarize the queue","Top 5 risky applications","Counties with rising climate risk","Suggest underwriting policy"].map(s => (
