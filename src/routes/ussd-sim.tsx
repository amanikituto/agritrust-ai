...
function UssdSim() {
  ...
  async function send(nextSteps: string[]) {
    ...
    try {
      const body = new URLSearchParams({
        sessionId, serviceCode: "*483*900#", phoneNumber: phone, text: nextSteps.join("*"),
      });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <SectionTitle eyebrow="Last-mile accessibility" title="USSD Simulator · *483*900#"
        sub="Test the offline lending flow that runs on any feature phone." />

      <Card>
        <label className="flex flex-col gap-1.5">
          <span className="text-xs uppercase tracking-wider text-muted-foreground">Caller phone (must exist in profiles)</span>
      ...
      <Card title="USSD screen" icon={Phone}>
        <div className="mb-3 flex items-center gap-2">
          <Tag label={isEnd ? "Session ended" : isCon ? "Awaiting input" : "Idle"} tone={isEnd ? "rose" : isCon ? "emerald" : "gold"} />
...
function SendInput({ onSend }: { onSend: (t: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <input value={val} onChange={(e) => setVal(e.target.value)} placeholder="text input"
        className="h-10 w-44 rounded-md border border-border bg-surface-elevated/60 px-3 text-sm" />
      <button onClick={() => { if (val) { onSend(val); setVal(""); } }}
